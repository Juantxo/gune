!(function(window, d3) {
    ...
    window.onload = function() {
	    d3.queue()
		.defer(d3.json, './data/gune_data.json')
	...
	function filter_data(database, data){
		
		console.log('data to filter TBD', data.ambito);
		
		let ambito_filter = database.filter(function(d){
			
			if(data.ambito === "0" || data.ambito === ""){
				
				return d; // TODOS los títulos.
				
			}else{
				return d.ambito_id === +data.ambito;
			}
		});
		
		let subambito_filter = ambito_filter.filter(function(d){
			
			if(data.subambito === "0" || data.subambito === ""){
				
				return d; // TODOS los títulos.
				
			}else{
				return d.subambito_id === +data.subambito;
			}
		...
		// mejora_filter es la información para pintar el árbol
		let mejora_filter = presencia_filter.filter(function(d){
			...
			switch(data.mejora){
				case "":
				case "0":
					return d; // pinta TODOS los títulos en los que exista presencia de alguna mejora.
					break;
				case "1":
					return d.i_d === 1;
					break;
		...
		console.log('data mejora_filter', mejora_filter);
	...
	function build_selects(data, dic){
		...
		const mejora_select = d3.select('#mejora_select').on('change',onchange);
		...
		function getOptionsData(val){
				// console.log('val', val, dictionary);
				
				let temp = dictionary.filter(function(d) { 
		...
		function onchange(){
			...
			if(this.dataset.select === "parent_select" ){
				...
				let options_data = getOptionsData(this.value);
			...
			console.log('selectValues::: ',selectValues, dictionary);
	...
	**************************************************************/
	...
	function get_results_numbers(data){
		...
		let result_numbers = {
			...
			"total": {"total": dataL, "number": 0 },
			"formacion": {"total": dataL, "number": 0 },
			"instalaciones_y_equipos": {"total": dataL, "number": 0 },			
			"investigacion": {"total": dataL, "number": 0 },			
			"transferencia": {"total": dataL, "number": 0 }			
		};
		let i = 0;
	...
	function draw_result_numbers(d){
	...
	function draw_table(data, values, dic){
		// data: todos los datos para hacer el nest
		// values: los valores seleccionados en los selects - {ambito: "0", subambito: "0", mejora: "6"}  - 
		// dic: el diccionario de ambitos
		
		console.log('draw_table', data, values, dic);
		...
		let results_partial = get_results_numbers(nested_data);
		//console.log('draw_table results_partial', results_partial);
		let select_info = get_the_select_info(values, dic); 
		...
		// actions
		draw_result_numbers(results_partial);
			...
			.html(function(d, i){
				...
				return '<div class="table-row table-border--gray padding-box--regular">'+
	                  ...
	                  '<div class="cell title">'+
	                     ...
	                     '</div>'+
	                  '</div>'+
	                  '<div class="cell star"><span class="star-text--container hidden-screen">I+D</span><span class="star-icon--container"><i class="fa star-icon ' + addClass_to_star(d, "i_d")  + '">&#xf005;</i></span></div>'+
	                  '<div class="cell star"><span class="star-text--container hidden-screen">Producción</span><span class="star-icon--container"><i class="fa star-icon ' + addClass_to_star(d, "produccion")  + '">&#xf005;</i></span></div>'+
	                  '<div class="cell star"><span class="star-text--container hidden-screen">Suministros y compras</span><span class="star-icon--container"><i class="fa star-icon ' + addClass_to_star(d, "suministro_y_compras")  + '">&#xf005;</i></span></div>'+
	                  '<div class="cell star"><span class="star-text--container hidden-screen">Almacenamiento y logística</span><span class="star-icon--container"><i class="fa star-icon ' + addClass_to_star(d, "almacenamiento_y_logistica")  + '">&#xf005;</i></span></div>'+
	                  '<div class="cell star"><span class="star-text--container hidden-screen">Marketing</span><span class="star-icon--container"><i class="fa star-icon ' + addClass_to_star(d, "marketing")  + '">&#xf005;</i></span></div>'+
	                  '<div class="cell star"><span class="star-text--container hidden-screen">Ventas</span><span class="star-icon--container"><i class="fa star-icon ' + addClass_to_star(d, "ventas")  + '">&#xf005;</i></span></div>'+
	                  '<div class="cell star"><span class="star-text--container hidden-screen">Servicios</span><span class="star-icon--container"><i class="fa star-icon ' + addClass_to_star(d, "servicios")  + '">&#xf005;</i></span></div>'+
	                  '<div class="cell star"><span class="star-text--container hidden-screen">Administración</span><span class="star-icon--container"><i class="fa star-icon ' + addClass_to_star(d, "administracion")  + '">&#xf005;</i></span></div>'+
	               '</div>'+
    ...
    let init = function(error, gune, dic) {
	    ...
	    init_table_keys();
	    draw_table(gune, {ambito: "0", subambito: "0", mejora: "0"}, dic);
	    init_ver_mas();
